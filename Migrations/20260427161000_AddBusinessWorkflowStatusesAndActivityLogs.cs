...
{
    public partial class AddBusinessWorkflowStatusesAndActivityLogs : Migration
    {
...
IF OBJECT_ID(N'[dbo].[station_activity_logs]', N'U') IS NOT NULL
BEGIN
    DROP TABLE [dbo].[station_activity_logs];
END
...
BEGIN
    DROP INDEX [IX_ChargingStations_PublicBusinessStatus] ON [dbo].[ChargingStations];
END

IF COL_LENGTH('dbo.ChargingStations', 'operation_status') IS NOT NULL
BEGIN
    ALTER TABLE [dbo].[ChargingStations] DROP COLUMN [operation_status];
END

IF COL_LENGTH('dbo.ChargingStations', 'system_status') IS NOT NULL
BEGIN
    ALTER TABLE [dbo].[ChargingStations] DROP COLUMN [system_status];
END
");
        }
    }
